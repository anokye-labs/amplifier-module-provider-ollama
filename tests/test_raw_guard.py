...
@pytest.mark.asyncio(loop_scope="function")
async def test_raw_events_suppressed_when_both_flags_false(
    make_provider, simple_request, mock_response
):
    """When debug=False and raw_debug=False, no :raw events should fire."""
    provider = make_provider(debug=False, raw_debug=False)
    provider.client.chat = AsyncMock(return_value=mock_response())
